...
Personalized installable builds are uploaded to the private
`arthurwangwsx-code/RootTools-runtime-releases` repository. Release assets
include the complete rootless `.deb`, the matching TrollStore-compatible
`.ipa`, `SHA256SUMS`, `SOURCE_BUILD_MANIFEST.json`, and `INSTALL.md`. The public
source repository does not publish credential-bearing binaries.
...
Use the reserved `installed` profile only for credentials already deployed on
the reference phone. Create an isolated candidate before building an upgrade:

```bash
Scripts/credential-profile.sh create candidate-v0.23.0-2
Scripts/credential-profile.sh compare installed candidate-v0.23.0-2
ROOTTOOLS_CREDENTIAL_PROFILE=candidate-v0.23.0-2 bash Scripts/build.sh
```

These commands print paths and SHA-256 fingerprints, never credential values.
...
RootTools binaries are personalized builds because the App and daemon contain matching device credentials. The release script permits binaries only in the dedicated private artifact repository, or as a maintainer-only draft in the public source repository. Candidate releases must use an isolated non-`installed` credential profile.
...
The current source/release candidate is **v0.23.0-2**. The last physically
qualified daemon remains **v0.21.0**; simulator and local fixture evidence do
not replace Dopamine/rootless device qualification. See
`docs/handoff/CURRENT_STATE.md` and `docs/validation/` for the exact boundary.
